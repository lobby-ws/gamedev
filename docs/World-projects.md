...
.hyperfy/targets.json       Local-only deploy targets (gitignored)
.hyperfy/targets.example.json  Shareable template for targets
...
- `.hyperfy/<worldId>/` is local runtime state.
...
- Use `.hyperfy/targets.json` for local targets (dev/staging/prod).
- Commit `.hyperfy/targets.example.json` as the shareable template.
- Use `hyperfy dev` for continuous sync (dev only).
- Use `hyperfy apps deploy <app>` for explicit staging/prod deploys.
